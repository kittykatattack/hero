...
type alias Model =
  ...
  , buttonLink : Data.ID
  , activeLink : Data.ID -- The actual link that will be used to load a new page
  }
...
model = 
  let
    ...
    choiceBoxes' =
      List.map (\string -> Choice.init string) choices'
  in
  { heading = "Adventure!"
  ...
  case msg of 
    ...
    UpdateChoices id choiceMsg ->
      let
        ...
        currentChoice' =
          List.Extra.getAt id choiceBoxes'
      ...
      if .currentMsg (.button currentChoice') == ImageButton.Click then
        (
          { model
              | choiceBoxes = choiceBoxes'
              , activeLink = choiceLink'
          }
        , Cmd.none
        )
    ...
    UpdateGameEventBox gameEventMsg ->
      ...
      let
        gameEventBox' = GameEvent.update gameEventMsg model.gameEventBox
        button = .button gameEventBox'
      in
  ...
  let
    ...
    contentBoxStyle =
      case model.pageType of
        Choice Top -> contentBoxStyleTop
        Choice Left -> contentBoxStyleLeft
        GameEvent Left -> contentBoxStyleLeft
        GameEvent Top -> contentBoxStyleTop

    storyBoxStyle = 
      case model.pageType of
        Choice Top -> storyBoxStyleTop
        Choice Left -> storyBoxStyleLeft
        GameEvent Top -> storyBoxStyleTop
        GameEvent Left -> storyBoxStyleLeft

    userMsgBoxStyle = 
      case model.pageType of
        Choice Top -> choiceBoxStyleTop
        Choice Left -> choiceBoxStyleLeft
        GameEvent Top -> choiceBoxStyleTop
        GameEvent Left -> choiceBoxStyleLeft

    arrowStyle =
      case model.pageType of
        Choice Top -> arrowStyleTop
        Choice Left -> arrowStyleLeft
        GameEvent Top -> arrowStyleTop
        GameEvent Left -> arrowStyleGameEventLeft

    -- The round arrow icon that connects the story box and event box
    arrowImage =
      case model.pageType of
        Choice Top -> "roundArrowRight.png"
        Choice Left -> "roundArrowDown.png"
        GameEvent Top -> "roundArrowRight.png"
        GameEvent Left -> "roundArrowDown.png"
