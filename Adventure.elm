...
init =
    let
        ...
        currentPage_ =
            getCurrentPage currentPageData_
    ...
    case msg of
        ...
        UpdatePage pageMsg ->
            let
                ...
                currentPage_ =
                    Tuple.first (Page.update pageMsg model.currentPage)

                -- fst (Page.update pageMsg (getCurrentPage currentPageData'))
                activeLink =
                    .activeLink currentPage_
                ...
                newPage =
                    if newPageRequested then
                        Tuple.first (Page.init currentPageData_)
                    else
                        currentPage_

                storyLevel =
                    getStoryLevel (.id currentPageData_)

                inventoryQuantities =
                    [ (.pillsLeft currentPageData_)
                    , (.transformations currentPageData_)
                    ]

                storyPhaseChapter =
                    .storyPhaseChapter currentPageData_
        ...
        UpdateInfoBox infoBoxMsg ->
            ...
            let
                ( infoBox_, fx ) =
                    InfoBox.update infoBoxMsg model.infoBox
            in
                ( { model
                    | infoBox = infoBox_
                  }
        ...
        FadeInNewPage ->
            ...
            in
                update (UpdatePage model_.pageMsg) model_
